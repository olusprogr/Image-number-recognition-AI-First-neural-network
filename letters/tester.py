...
for i, prob in enumerate(output_a):
    print(f"Wahrscheinlichkeit für {i}. {chr(i + 65)}: {prob:.4f}")

predicted_index = np.argmax(output_a)
predicted_later = chr(predicted_index + ord('A'))
print(f"Vorhergesagte Ziffer: {predicted_later}")

true_label_char = input("Was war der tatsächliche Buchstabe? (A-Z): ").upper()
true_label = ord(true_label_char) - ord('A')
